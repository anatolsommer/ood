    ...
    request('status', {app:appName}, function(res) {
      var tbl=zygon([
        ...
        {name:'CPU', size:6, align:'right', format:function(p) {
          return p===-1 ? 'N/A'.grey : p.toFixed(1)+'%';
        }},
...
app.command('ssl <app>')
  .description(
    'Change ssl settings for an app\n\n'+
    '  Hint: Using --cert, --key or --auto will enable ssl for this app!'
  )
//.option('-a, --auto', 'Get a free certificate from letsencrypt.org')
  .option('-k, --key <file>', 'Path to private key file')
  .option('-c, --cert <file>', 'Path to certificate file')
  .option('-i, --ca <file>', 'Path to issuer certificate file (optional)')
  .option('-d, --disable', 'Disable ssl for this app')
  .option('-e, --enable', 'Enable ssl for this app')
  .action(function(app, opts) {
    // TODO
  })
  .on('--help', examples([
//  'ood ssl example.com --auto',
    'ood ssl example.com -k /root/ssl/example.com/key.pem -c /root/ssl/example.com/cert.pem',
    'ood ssl example.com --disable',
  ]));
