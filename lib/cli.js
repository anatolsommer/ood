...
var app=require('commander'), zygon=require('zygon'),
...
process.on('uncaughtException', function(err) {
  ...
  if (err.code==='EACCES') {
    ...
    console.log('Use sudo or consult: https://oodjs.com/docs/security');
    process.exit();
  } else {
...
app.command('redirect <host> [target]')
  ...
  .action(function(host, target, opts) {
    var args;
    if (!opts) {
      opts=target;
      target=null;
  ...
  .on('--help', examples([
    ...
    'ood redirect example.com https://example.com',
...
app.command('config')
  ...
  .action(function(val, opts) {
    ...
    request('config', change, function(res) {
      ...
      if (opts.get) {
        val=JSON.stringify(res.value, null, 2).replace(/\n/g, '\n  ');
        console.log('\n  '+(opts.get===true ? '' : opts.get+' = ')+val+'\n');
      }
    });
  })
  .on('--help', examples([
