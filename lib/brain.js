...
ood.loadModule=function(modName) {
  var mod=ood.mods[modName], config=ood.config.get('mod:'+modName);
  ...
  if (mod.type==='app') {
    ...
    config.env.NODE_ENV=config.env.NODE_ENV || 'production';
    mod.master=child.fork(
      __dirname+'/modContainer',
      {env:config.env, execArgv:['--expose-gc'], silent:true}
    );
    mod.master.send({
      cmd:'load',
      modName:modName,
