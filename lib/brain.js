...
ood.updateProxy=function(pid) {
  ...
  Object.keys(ood.mods).forEach(function(modName) {
    ...
    if (mod.enabled && mod.proxyPath) {
      hosts.mod[mod.proxyPath]={modName:modName, port:mod.port};
    }
  });
  Object.keys(ood.apps).forEach(function(appName) {
    var port, config;
...
ood.loadModule=function(modName) {
  ...
  if (mod.type==='app') {
    ...
    mod.master=child.fork(__dirname+'/modContainer', {silent:true});
    mod.master.send({
      cmd:'load',
      modName:modName,
    ...
    mod.master.on('exit', function(code) {
      ...
      if (code!==0) {
        log.error('Module crashed, disabling it.', {module:modName, exitCode:code});
        mod.enabled=false;
        config.set('mod:'+modName+':enabled', false);
      }
    });
    if (mod.proxyPath) {
