...
function forkWorker() {
  ...
  worker.on('message', function(msg) {
    ...
    } else if (appName==='proxy' && msg.type==='accessLog') {
      process.send({type:'accessLog', app:msg.app, mod:msg.mod, log:msg.log});
    }
  });
...
function appLog(level, msg, workerId, pid) {
  var details={workerId:workerId, pid:pid};
  msg=msg.toString();
  if (appName==='proxy' && msg[0]==='{') {
    details=JSON.parse(msg);
    msg=details.msg;
  }
  process.send({
    type:'appLog',
